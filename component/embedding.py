...
class OpenAIembedding:
    ...
    def __init__(self, model: str = "text-embedding-3-small"):
        ...
        self.embedding = OpenAIEmbeddings(
            model=model,
            # 移除 openai_api_key 参数，让它自动从环境变量获取
        )
...
class QwenEmbedding:
    def __init__(self, model_path="/path/to/Qwen2.5-1.5B-Instruct/"):
        self.tokenizer = AutoTokenizer.from_pretrained(model_path)
        self.model = Qwen2Model.from_pretrained(model_path)
        self.model = self.model.to("cuda").half()  # 如果使用GPU
        
    def get_embedding(self, text):
        # 对输入文本进行编码
        inputs = self.tokenizer(text, return_tensors="pt", padding=True, truncation=True, max_length=512)
        inputs = {k: v.to("cuda").half() for k, v in inputs.items()}  # 移至GPU并转换为half精度
